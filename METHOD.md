# ABCD × DBNT × KISS — Method Reference

This document is the canonical, harness-neutral specification of the ABCD × DBNT × KISS improvement loop.

Copy or load this document into any AI agent harness:
- **Claude Code**: see `skills/abcd-dbnt-kiss/SKILL.md`
- **Codex**: see `adapters/codex/AGENTS.md`
- **Cursor**: see `adapters/cursor/rules.md`
- **Any other harness**: see `adapters/generic/HOWTO.md`
...
- Rules or lessons directories: `rules/`, `docs/lessons/`, `.dbnt/rules/` (or any `rules/successes` / `rules/failures` pair nearby)
- An existing `CLAUDE.md`, `AGENTS.md`, or `MEMORY.md` that names a capture convention
- Whether the method is installed globally or project-locally
...
**Step 3 — Write a config file** at `<harness-skill-dir>/abcd-dbnt-kiss/config.yaml`:
...
Where `<harness-skill-dir>` is wherever your harness loads skill or instruction files. For example:
- Claude Code global: `~/.claude/skills/abcd-dbnt-kiss/config.yaml`
- Claude Code project-local: `.agents/skills/abcd-dbnt-kiss/config.yaml`
- Codex: `<your-project>/.dbnt/abcd-dbnt-kiss/config.yaml`
- Cursor: `.cursor/abcd-dbnt-kiss.yaml`

**Step 4 — Confirm in one line** what was set and how to change it:

> Config written to `<harness-skill-dir>/abcd-dbnt-kiss/config.yaml` — edit that file to change any setting.
...
Config file path: `<harness-skill-dir>/abcd-dbnt-kiss/config.yaml`.
...
`DBNT_DIR` must be an explicit caller-selected, project-local artifact directory. If it is unset, do not write a capture artifact: return the proposed path as `PLANNED`, mark the capture `held_unproven`, and ask the caller to select an artifact root. Never fall back to a user-global directory without explicit caller selection.
